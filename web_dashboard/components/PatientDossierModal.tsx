...
import { X, CheckCircle2, AlertTriangle, Printer, User, MapPin, Briefcase, Calendar } from "lucide-react";
import { PatientScreening, updateSpecialistNotes } from "@/lib/mock_database";
...
export const PatientDossierModal: React.FC<PatientDossierModalProps> = ({
  ...
  const [isSignedOff, setIsSignedOff] = useState(screening.isSignedOff || false);
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-8 max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-slate-900 text-lg">{screening.name}</h3>
                <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${getBadgeClass(screening.triageClass)}`}>
                ...
                {isSignedOff && (
                  <span className="bg-blue-100 text-blue-800 border border-blue-200 text-xs font-semibold px-2 py-0.5 rounded-full flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" /> Specialist Signed Off
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-500 flex items-center gap-4 mt-0.5">
                <span>ABHA ID: <strong className="text-slate-700">{screening.abhaId}</strong></span>
                <span>• Joint: <strong className="text-teal-700">{screening.jointName}</strong></span>
                <span>• Screened: {screening.screenedAt.split("T")[0]}</span>
              </p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-bold text-slate-800">{screening.age} Yrs / {screening.gender}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-bold text-slate-800">{screening.district}, {screening.state}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-bold text-slate-800">{screening.occupation}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-bold text-slate-800">VAS {screening.vasScore}/10 (WOMAC: {screening.surveyScore.toFixed(0)})</p>
              </div>
            </div>
          </div>

          {/* Dynamic 100 Hz Kinematics Excursion Curve */}
          <div>
          ...
          {/* Biomarkers Matrix & Camera Posture */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 bg-white">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-slate-100">
                  <span className="text-slate-500">Active Range of Motion (Δθ)</span>
                  <span className="font-bold text-slate-800">{screening.rom.toFixed(1)}°</span>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-100">
                  <span className="text-slate-500">Hesitation Arc (θ_crit)</span>
                  <span className={`font-bold ${screening.thetaCrit > 0 ? "text-rose-600" : "text-slate-800"}`}>
                    {screening.thetaCrit > 0 ? `${screening.thetaCrit.toFixed(1)}° (Pain Arc)` : "None"}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-100">
                  <span className="text-slate-500">Peak Angular Velocity (ω)</span>
                  <span className="font-bold text-slate-800">{screening.peakOmega.toFixed(1)} °/s</span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-slate-500">Mean Angular Jerk</span>
                  <span className="font-bold text-slate-800">{screening.meanJerk.toFixed(0)} °/s³</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-slate-200 bg-white">
              ...
              <div className="mt-3 flex items-center gap-2 text-[11px] text-slate-500">
                ...
                <span>Computed via on-device ML Kit coronal pose analysis.</span>
              </div>
            </div>
          </div>

          {/* Specialist Clinical Notes & Prescription */}
          <div className="p-4 rounded-xl bg-teal-50/60 border border-teal-200">
            ...
            <textarea
              ...
              placeholder="Enter clinical assessment notes, pharmacotherapy recommendations (e.g. Paracetamol / NSAIDs), intra-articular injection schedule, or referral to District Hospital..."
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={isSaving}
              className="flex items-center gap-2 px-5 py-2 bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold rounded-xl shadow-sm shadow-teal-600/30"
            >
              <CheckCircle2 className="w-4 h-4" />
              {isSaving ? "Saving..." : isSignedOff ? "Update Sign-Off" : "Authorize Referral Sign-Off"}
            </button>
          </div>
        </div>
      </div>
